...
class BluetoothManager: NSObject {
    ...
    var writableCharacteristics: [UUID: CBCharacteristic] = [:]
    
    init(serviceUUID: String, serviceUUID2: String, characteristicUUID: String, characteristicUUID2: String, characteristicUUID3: String, characteristicUUID4: String, characteristicUUID5: String, characteristicUUID6: String, characteristicUUID7: String, characteristicUUID8: String, delegate: BluetoothManagerDelegate?) {
        self.serviceUUID = serviceUUID
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        print("Discovered \(name)")
        print("Peripheral UUID: \(peripheral.identifier)")
        if name == "BLE_DEVICE" || name == "BLE_DEVICE2" {
            ...
            if connectedPeripherals.count >= 2 { // 2デバイスに接続したらスキャンを停止
                centralManager.stopScan()
            }
        }
    }
